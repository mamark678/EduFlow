...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <form method="POST" action="{{ route('admin.forums.update', $forum) }}" class="p-6">
                @csrf
                @method('PUT')
                <!-- Forum Name -->
                <div class="mb-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-6 gap-3">
                        @php
                        $icons = ['💬', '📚', '⭐', '🔧', '🎓', '💼', '👨‍🏫', '📢', '💡', '🎯', '🚀', '🌟', '📖', '🎨', '💻', '🎵', '🏃‍♂️', '🍕', '🌍', '🎪', '🎭', '🏆', '💎', '🔥'];
                        $selectedIcon = old('icon', $forum->icon ?? $icons[0]);
                        @endphp
                        @foreach($icons as $icon)
                        <label class="relative">
                            <input type="radio" name="icon" value="{{ $icon }}" class="sr-only" {{ $selectedIcon === $icon ? 'checked' : '' }}>
                            <div class="w-12 h-12 border-2 border-gray-200 rounded-lg flex items-center justify-center text-xl cursor-pointer hover:border-blue-300 transition-colors icon-option {{ $selectedIcon === $icon ? 'border-blue-500 bg-blue-50' : '' }}">
                                {{ $icon }}
                            </div>
                        </label>
                        @endforeach
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-6 gap-3">
                        ...
                        $selectedColor = old('color', $forum->color ?? $colors[0]);
                        @endphp
                        @foreach($colors as $color)
                        <label class="relative">
                            <input type="radio" name="color" value="{{ $color }}" class="sr-only" {{ $selectedColor === $color ? 'checked' : '' }}>
                            <div class="w-12 h-12 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors color-option {{ $selectedColor === $color ? 'border-blue-500 ring-2 ring-blue-200' : '' }}" style="background-color: {{ $color }};"></div>
                        </label>
                        @endforeach
                    </div>
                    @error('color')
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="order" id="order" min="0" value="{{ old('order', $forum->order ?? 0) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="0">
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $forum->is_active ?? false) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                ...
                <div class="flex justify-end space-x-3">
                    <a href="{{ route('admin.forums.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                        Cancel
                    </a>

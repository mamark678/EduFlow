...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <form method="POST" action="{{ route('admin.forums.store') }}" class="p-6">
                @csrf
                <!-- Forum Name -->
                <div class="mb-6">
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Forum Name</label>
                    <input type="text" name="name" id="name" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Study Tips & Strategies" required value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-6 gap-3">
                        @php
                        $icons = ['💬', '📚', '⭐', '🔧', '🎓', '💼', '👨‍🏫', '📢', '💡', '🎯', '🚀', '🌟', '📖', '🎨', '💻', '🎵', '🏃‍♂️', '🍕', '🌍', '🎪', '🎭', '🏆', '💎', '🔥'];
                        $oldIcon = old('icon', $icons[0]);
                        @endphp
                        @foreach($icons as $icon)
                        <label class="relative">
                            <input type="radio" name="icon" value="{{ $icon }}" class="sr-only" {{ $oldIcon == $icon ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-6 gap-3">
                        ...
                        $oldColor = old('color', $colors[0]);
                        @endphp
                        @foreach($colors as $color)
                        <label class="relative">
                            <input type="radio" name="color" value="{{ $color }}" class="sr-only" {{ $oldColor == $color ? 'checked' : '' }}>
                            <div class="w-12 h-12 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors color-option" style="background-color: {{ $color }};"></div>
                        </label>
                        @endforeach
                    </div>
                    @error('color')
                ...
                <div class="mb-6">
                    ...
                    <input type="number" name="order" id="order" min="0" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" placeholder="0" value="{{ old('order', 0) }}">
